...
char* indexes(const char* path)
{
    ...
    if(fp != NULL)
    {
		dir = "/path/to/a/directory/index.php";
		return dir;
	}
	else
	{
		...
		if(fp != NULL)
		{
			dir = "/path/to/a/directory/index.html";
			return dir;
		}
	}
	fclose(fp);
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char** copyofpath = malloc(sizeof(char**));
    strcpy(*copyofpath, abs_path);
    for (int i = 0, n = strlen(*copyofpath); i < n; i++)
    {
        for (int j = 0, m = strlen(*copyofpath) - i; j < m; j++)
        {
            if (*copyofpath[i] == '/' && *copyofpath[i+1] == '?')
				query[j] = *copyofpath[i+2];
            if (*copyofpath[i] == '/' && *copyofpath[i+1] != '?') 
				query[j] = *copyofpath[i+1];
            if (query[j] == '\0')
				break;
        }
    }
